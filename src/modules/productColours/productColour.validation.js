...
const productCode = z.string().trim().min(1).max(100).transform((value, context) => {
...
module.exports = {
  createProductColourSchema: { body: z.object({ productId: id('Product ID'), colourId: id('Colour ID'), productCode, status: status.optional() }).strict() },
