...
const USAGE =
  'node scripts/importCatalogMigration.js <xlsx-file> --yes [--sample-size N]\n' +
  '   or: node scripts/importCatalogMigration.js --file <xlsx-file> --yes [--sample-size N]';
...
const parseArguments = (args) => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--yes') {
      if (confirmed) {
        throw new CliError('Specify --yes only once');
      }
  ...
  if (!confirmed) {
    throw new CliError('Import requires the explicit --yes confirmation flag');
  }
...
const importCannotProceed = (report) => {
...
const toPublicErrorMessage = (error, filePath) => {
  ...
  let message = error?.message || 'Catalog migration import failed';
...
const main = async () => {
  ...
  try {
    ...
    const { importCatalogMigration } = require('../src/modules/catalogMigration/catalogMigration.service');

    await mongoose.connect(process.env.MONGODB_URI);

    report = await importCatalogMigration(buffer, {
