...
test('finalized Product, ProductColour, and immutable SKU catalog is enforced', { timeout: 60000 }, async () => {
  ...
  try {
    ...
    const admin = await User.create({ name: 'Catalog Admin', email: 'final.catalog@example.test', password, role: 'admin', status: 'active' });
    ...
    assert.equal(black.skus[0].sku, 'custom_sku_01');
    ...
    const collisionProduct = await request('/products', {
      method: 'POST', token,
      body: { name: 'Collision', categoryId: category._id, subCategoryId: subCategory._id, fitId: fit._id, fabricId: fabric._id, mrpPerPieceMinor: 10000, productColours: [{ colourId: colourWhite._id, productCode: 'another_code', skus: [{ sizeSetId: alphaSizes._id, sku: 'custom_sku_01' }] }] },
    });
    assert.equal(collisionProduct.status, 409);
    assert.equal(await Product.countDocuments(), productCountBefore);
    assert.equal(await ProductColour.countDocuments({ productCode: 'another_code' }), 0);
  } finally {
